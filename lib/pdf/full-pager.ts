...
export async function buildFullPagerPdf(records: PantryRecord[]): Promise<Uint8Array> {
  ...
  const doc = await renderModel(model, {
    ...
    subtitle: `${records.length} locations · ${countEntries(model)} listings by day · grouped by area and city. Hours change — call ahead.`,
  });
  return doc.save();
}
